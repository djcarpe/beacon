defmodule Beacon.MediaLibrary do
  ...
  @doc """
  Returns a presigned, client-reachable URL for `asset` — bypassing the
  Phoenix proxy at `/__beacon_media__/<file>`.

  This is the "I want the browser to fetch bytes directly from S3"
  path. The URL is generated by `Beacon.MediaLibrary.Provider.S3.Signed`
  and respects the
  `:beacon, :media_library_presigned_url_endpoint` config knob, so the
  embedded host can be a client-reachable public endpoint even when
  uploads (and the in-cluster Provider.S3 proxy fetch) use a different
  in-cluster ServiceName.

  Returns the URL string on success, or `nil` if the asset has no S3
  provider mapping. The companion HTTP route
  `/__beacon_media_presigned__/:file_name` (served by
  `Beacon.Web.MediaLibraryController.presigned/2`) issues a 302 redirect
  to this URL — use that when the template can't compute the URL itself.
  """
  @spec presigned_url_for(Asset.t()) :: String.t() | nil
  def presigned_url_for(%Asset{} = asset) do
    case Map.get(asset.keys || %{}, Beacon.MediaLibrary.Provider.S3.provider_key()) do
      key when is_binary(key) and key != "" ->
        Beacon.MediaLibrary.Provider.S3.Signed.url_for(asset)
  ...
  def presigned_url_for(_), do: nil
